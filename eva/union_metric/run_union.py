...
flags.DEFINE_boolean("use_reconstruction", True, "Whether to use original bert")
...
flags.DEFINE_string("bert_config_path", "./model/uncased_L-12_H-768_A-12/bert_config.json", "The config json file corresponding to the pre-trained BERT model. This specifies the model architecture.")
flags.DEFINE_string("train_file_name", "train", "file name prefix for training")
flags.DEFINE_string("dev_file_name", "dev", "file name prefix for validation")
flags.DEFINE_string("test_file_name", "test", "file name prefix for testing")
...
## Other parameters
flags.DEFINE_string("bert_config_file", "%s/bert_config.json"%(FLAGS.bert_config_path), "The config json file corresponding to the pre-trained BERT model. This specifies the model architecture.")
flags.DEFINE_string("vocab_file", "%s/vocab.txt"%(FLAGS.bert_config_path),"The vocabulary file that the BERT model was trained on.")
...
flags.DEFINE_float("num_train_epochs", 200, "Total number of training epochs to perform.")
...
def _read_st(input_file):
  ...
  if FLAGS.use_reconstruction:
    if "positive" in input_file:
      ...
      ref_file = input_file.replace("negative", "positive")
      with tf.gfile.Open(ref_file+".txt", "r") as fin:
        st_ref = _read_line(fin)
  else:
...
class GenStoryProcessor(DataProcessor):
  def get_test_examples(self):
    ...
    example = self._create_examples(
          _read_st(os.path.join(FLAGS.data_dir, "%s_%s"%(FLAGS.test_file_name, "positive"))), "pred")
    print("pred", len(example))
    return example
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      text_ref = [tokenization.convert_to_unicode(s) for s in line["ref"]] if FLAGS.use_reconstruction else None
      examples.append(
          InputExample(guid=guid, text_a=text_a, label=0, ref=text_ref))
    return examples

class StoryClassiferProcessor(DataProcessor):
  def __init__(self):
    self.name_list = ["negative"] + ["positive"]
  ...
  def get_train_examples(self):
    ...
    for f in self.name_list:
      example += self._create_examples(
            _read_st(os.path.join(FLAGS.data_dir, "%s_%s"%(FLAGS.train_file_name, f))), "train-%s"%f)
  ...
  def get_dev_examples(self):
    ...
    for f in self.name_list:
      example += self._create_examples(
            _read_st(os.path.join(FLAGS.data_dir, "%s_%s"%(FLAGS.dev_file_name, f))), "dev-%s"%f)
    np.random.shuffle(example)
  ...
  def get_test_examples(self):
    ...
    for f in self.name_list:
      example += self._create_examples(
            _read_st(os.path.join(FLAGS.data_dir, "%s_%s"%(FLAGS.test_file_name, f))), "test-%s"%f)
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      text_ref = [tokenization.convert_to_unicode(s) for s in line["ref"]] if FLAGS.use_reconstruction else None
      examples.append(
          InputExample(guid=guid, text_a=text_a, label=line["label"], ref=text_ref))
    return examples
...
def eval(processor, estimator, label_list, tokenizer, num=0, name_list=None, map_name_func=None):
  for name in name_list:
    eval_examples = map_name_func[name]()
...
def main(_):
  ...
  if FLAGS.do_train:
    train_examples = map_name_func["train"]()
    num_train_steps = int(
  ...
  if FLAGS.do_predict:
    for name in ["test"]:
      predict_examples = map_name_func[name]()
      ...
      for opt_name in opt_name_list:
        output_name = "%s_%s_%s.txt"%(FLAGS.test_file_name, opt_name, FLAGS.init_checkpoint.split("/")[-1])
